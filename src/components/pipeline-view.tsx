...
function WordPicture({ session }: { session: WordSession }) {
...
export function PipelineView({ state, dispatch, devMode, onEnd, input, onInputChange }: Props) {
  const { activity } = state;
    ...
    <div className="conv-layout">
      ...
      <div ref={messagesRef} className="conv-messages" style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
        {activity.kind === "loading" ? (
          ...
          <>
            {session && <WordPicture session={session} />}
